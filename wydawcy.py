...
logging.basicConfig(
    filename=r"C:\Users\genma\Desktop\logi wydawcy\automat.log",
    level=logging.INFO,
...
try:
    df = pd.read_excel(r'C:\Users\genma\Desktop\automat\wydawcy.xlsx')
    logging.info("Wczytano plik")
    df = df.where(df.notna(),None)
    logging.info(f"Liczba rekordów: {len(df)}")

    insert_sql = "INSERT INTO wydawcy (Nazwa_wydawcy, ID_Kraj, Rok_zalozenia) values (?,?,?)"
