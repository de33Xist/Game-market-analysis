...
logging.basicConfig(
    filename=r"C:\Users\genma\Desktop\logi ceny\automat.log",
    level=logging.INFO,
...
try:
    df = pd.read_excel(r'C:\Users\genma\Desktop\automat\ceny.xlsx')
    logging.info("Wczytano plik")
    df = df.where(df.notna(),None)
    logging.info(f"Liczba rekordów: {len(df)}")


    insert_sql = "INSERT INTO ceny (ID_Gry,Cena_PLN_PC,Cena_PLN_KONSOLA) values (?,?,?)"
